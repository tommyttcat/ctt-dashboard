// scripts/backtest/score-vcp.ts — what each replayed VCP base did next.
//
// Run from trade-dash after replay-vcp.ts:  npx tsx scripts/backtest/score-vcp.ts
// Reads  CTT/backtest-data/replay/vcp_registry.jsonl + the bar cache
// Writes CTT/backtest-data/replay/vcp_outcomes.jsonl
//
// RULES — fixed 11 Sep 2026 BEFORE any VCP outcome was looked at.
//
//   Entries
//     pivot   The scan's own trade: the pivot must trade within ENTRY_WINDOW
//             sessions of the flag. Fill = pivot, or the open if it gaps above.
//             Stop = the final contraction's low (2% floor), target = 2R.
//             A base is flagged daily while it lives, so this is measured from
...
function main() {
  ...
  const lines = fs.readFileSync(path.join(REPLAY, 'vcp_registry.jsonl'), 'utf8').trim().split('\n');
  const out = fs.createWriteStream(path.join(REPLAY, 'vcp_outcomes.jsonl'));
  ...
  for (const line of lines) {
    ...
    const trigger: number | null = e.trigger ?? null;
    const stop: number | null = e.stop ?? null;
    ...
    if (trigger == null || stop == null || !(trigger > stop)) {
      ...
      entries.pivot = ei < 0 ? { status: 'no_trade' } : simulate(cache, id, s, ei, fill, stop, e.target ?? null);
  ...
  out.end();
  console.log(`scored ${lines.length} VCP rows in ${((Date.now() - t0) / 1000).toFixed(0)}s:`, JSON.stringify(tally));
}

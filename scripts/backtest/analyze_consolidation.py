"""scripts/backtest/analyze_consolidation.py — slice the 10/21 Consolidation backtest.

Run from trade-dash after replay-consolidation + score-consolidation:  python3 scripts/backtest/analyze_consolidation.py [--all-rows]
Reads  CTT/backtest-data/replay/vcp_{registry,outcomes}.jsonl
Writes CTT/backtest-data/replay/consol_summary.json  (and prints the headline tables)
...
def load():
    reg = {}
    with open(os.path.join(REPLAY, 'consol_registry.jsonl')) as f:
        for line in f:
            r = json.loads(line)
            reg[(r['date'], r['ticker'])] = r
    rows = []
    with open(os.path.join(REPLAY, 'consol_outcomes.jsonl')) as f:
        for line in f:
            o = json.loads(line)
...
SLICES = {
    ...
    'coilRatio': lambda r: bucket(r['coilRatio'], [2.0, 2.5, 3.0, 3.5], ['<2', '2-2.5', '2.5-3', '3-3.5', '3.5-4']),
    'range10Pct': lambda r: bucket(r['range10Pct'], [5, 8, 11], ['<5', '5-8', '8-11', '11-14']),
    'coilDays': lambda r: bucket(r['coilDays'], [5, 10, 20], ['<5', '5-10', '10-20', '20+']),
    'priorMovePct': lambda r: bucket(r['priorMovePct'], [15, 30, 60], ['<15', '15-30', '30-60', '60+']),
    'bvrRatio': lambda r: bucket(r['bvrRatio'], [0.6, 0.8, 1.0], ['<.6', '.6-.8', '.8-1', '1+']),
    'ema1021GapPct': lambda r: bucket(r['ema1021GapPct'], [0, 1, 2.5], ['<0', '0-1', '1-2.5', '2.5+']),
    'rsRating': lambda r: bucket(r['rsRating'], [70, 80, 90], ['50-69', '70-79', '80-89', '90+']),
    'atrPct': lambda r: bucket(r['atrPct'], [3, 4.5, 6], ['<3', '3-4.5', '4.5-6', '6+']),
    'adrPct': lambda r: bucket(r['adrPct'], [4, 5.5, 7], ['3-4', '4-5.5', '5.5-7', '7+']),
    'distToEma10': lambda r: bucket(abs(r['distToEma10']) if r.get('distToEma10') is not None else None, [1, 2.5, 4], ['<1', '1-2.5', '2.5-4', '4-5']),
    'pctOffHigh': lambda r: bucket(r['pctOffHigh'], [3, 7, 11], ['<3', '3-7', '7-11', '11-15']),
    'mf': lambda r: bucket(r['mf'], [45, 55, 65], ['<45', '45-55', '55-65', '65+']),
    'stage': lambda r: (r.get('stage') or 'n/a')[:7],
    'blueDot': lambda r: r.get('blueDot'),
    'chop14': lambda r: bucket(r['chop14'], [50, 61.8, 70], ['<50', '50-62', '62-70', '70+']),
    'stochK': lambda r: bucket(r['stochK'], [25, 50, 75], ['<25', '25-50', '50-75', '75+']),
    'priceAsTraded': lambda r: bucket(r['price'], [10, 25, 60, 120], ['<10', '10-25', '25-60', '60-120', '120+']),
    'avgDollarVolM': lambda r: bucket(r['avgDollarVolM'], [25, 75, 250], ['<25M', '25-75M', '75-250M', '250M+']),
...
def main():
    ...
    with open(os.path.join(REPLAY, 'consol_summary.json'), 'w') as f:
        json.dump(summary, f, indent=1)

    a = summary['all']['ALL']
    print(f"CONSOL  {summary['population']}  {dates[0]} → {dates[-1]}  split {cut}")
    print(f"coils {a['n']}  matured {a['matured']}  broke out {a['traded']}  breakout rate {a['triggerRate']}%")

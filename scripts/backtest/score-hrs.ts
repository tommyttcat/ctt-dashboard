// scripts/backtest/score-hrs.ts — what each Hidden RS name did next.
//
// Run from trade-dash after replay-hrs.ts:  npx tsx scripts/backtest/score-hrs.ts
// Writes CTT/backtest-data/replay/hrs_outcomes.jsonl
//
// RULES — fixed 11 Sep 2026 BEFORE any HRS outcome was read.
//   Entries
//     open1      Buy the next session's open. Stop = the 10-day low as of the
//                flag (a quiet name's own low is often inches away, and a stop
//                inside that noise would measure the spread, not the setup).
//                Target = fill + 2R.
...
function main() {
  ...
  const lines = fs.readFileSync(path.join(REPLAY, 'hrs_registry.jsonl'), 'utf8').trim().split('\n');
  const out = fs.createWriteStream(path.join(REPLAY, 'hrs_outcomes.jsonl'));
  ...
  for (const line of lines) {
    ...
    const stop: number | null = e.low10 ?? null;
  ...
  out.end();
  console.log(`scored ${lines.length} HRS rows in ${((Date.now() - t0) / 1000).toFixed(0)}s:`, JSON.stringify(tally));
}

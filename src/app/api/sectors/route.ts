...
export async function GET() {
  ...
  const sectors = finalSectors
    .map((s: any) => ({ sector: s.sector, changesPercentage: parseFloat(s.averageChange ?? s.changesPercentage ?? 0) }))
    .sort((a, b) => b.changesPercentage - a.changesPercentage)
    .slice(0, 11);

  const industries = finalIndustries
    .map((i: any) => ({ industry: i.industry, changesPercentage: parseFloat(i.averageChange ?? i.changesPercentage ?? 0) }))
    .sort((a, b) => b.changesPercentage - a.changesPercentage)
    .slice(0, 10);

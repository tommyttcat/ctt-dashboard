...
export async function GET(request: Request) {
  ...
  try {
    ...
    const processedSnapshot = rawSnapshot.map((t: any) => {
      // SAFE DEFAULTS: Guarantees no null/undefined math errors during pre-market
    ...
    const enrichCandidate = async (t: any) => {
      ...
      // SAFE DEFAULTS for Enrichment Math
      const price = Number(t._livePrice) || 0;
      const vol = Number(t._liveVol) || 0;
      const chgPct = Number(t._liveChg) || 0;
      const vwap = Number(t._liveVwap) || price;
      ...
      // Market Cap Safe Default
      const marketCap = Number(details?.results?.market_cap) || 0;

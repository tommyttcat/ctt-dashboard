...
const ETF_TARGET_MAP: Record<string, string> = {
  ...
  'DIA': 'DIA', 'VOO': 'SPX', 'VTI': 'Total Market'
};
...
export default function TopMovers() {
    ...
    const fetchMarketData = async () => {
      try {
        ...
        if (isMounted) {
          ...
          // HARD FILTER: Require Market Cap >= $10,000,000 (Ignored for ETFs)
  ...
  // REFACTORED TO EXTRACT AND OVERRIDE SESSIONS TO "CLOSED" ON THE WEEKEND
  ...
  const displaySession = useMemo(() => {
    const estDate = new Date(new Date().toLocaleString("en-US", { timeZone: "America/New_York" }));
    const day = estDate.getDay();
    if (day === 0 || day === 6) return 'Closed'; // Weekend short-circuit
    return session;
  }, [session]);
                  ...
                  sortedStocks.map((row, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        {/* CATALYST CELL WITH HOVER REMOVED AND CLICKABLE NATIVE TOOLTIP */}

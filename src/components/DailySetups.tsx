...
export default function DailySetups() {
  // THE FIX: Pull session, timestamp, and master data directly from the Context Provider
  ...
  // Format the context universe to strictly match the Daily Setups logic
  const setups: SetupData[] = useMemo(() => {
    if (!topMovers || topMovers.length === 0) return [];
    
    const mappedData = topMovers.map((item: any) => ({
      ticker: item.ticker || '—',
      name: item.name || '',
    ...
    // STRICT FILTER ENFORCEMENT: Price > $1, Gain > 4%, Cap > $20M, sorted by Dollar Volume
    return mappedData

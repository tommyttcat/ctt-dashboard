                  ...
                  sortedStocks.map((row, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-3 py-3 text-xs text-slate-300 font-medium whitespace-nowrap text-left">
                        ...
                        <td className={`px-3 py-3 text-xs font-bold whitespace-nowrap text-left ${isPositive ? 'text-emerald-400' : 'text-rose-400'}`}>{isPositive ? '+' : ''}{row.changePct.toFixed(2)}%</td>
                        <td className="px-3 py-3 text-xs text-slate-400 font-medium whitespace-nowrap text-left">{formatNumber(row.vol)}</td>
                        <td className="px-3 py-3 text-xs text-slate-400 font-medium whitespace-nowrap text-left">{formatCurrency(row.dVol)}</td>
                        <td className={`px-3 py-3 text-xs font-bold whitespace-nowrap text-left ${getRvolColor(row.rvol)}`}>{row.rvol ? `${row.rvol.toFixed(1)}x` : '—'}</td>
                        {!isEtfTab && (
                          <>
                            <td className={`px-3 py-3 text-xs font-bold whitespace-nowrap text-left ${getFloatColor(row.float)}`}>{formatNumber(row.float)}</td>
                            <td className={`px-3 py-3 text-xs font-bold whitespace-nowrap text-left ${getShortColor(row.shortPct)}`}>{row.shortPct ? `${row.shortPct.toFixed(1)}%` : '—'}</td>
                            <td className="px-3 py-3 text-xs text-slate-400 font-medium whitespace-nowrap text-left">{formatNumber(row.mktCap)}</td>
                          </>
                        )}
